<template>
  ...
  <div class="home-work-context">
    <el-table :data="testData" style="width: 100%">
      ...
      <el-table-column prop="test_name" label="测验名称" width="170" />
      <el-table-column prop="test_starttime" label="开始时间" width="170" />
      <el-table-column prop="test_endtime" label="结束时间" width="170" />
      <el-table-column prop="test_score" label="得分" width="170" />
      <el-table-column label="操作">
        <button @click="doWork()">去测验</button>
...
let testData = ref([
  {
    ...
    course_title: "数据结构",
    test_name: "数组",
    test_starttime: "2021-11-08",
    test_endtime: "2021-11-15",
    test_score: 50,
  },
]);
...
let formatData = (val: any) => {
  return val.map((item: any) => ({
    test_id: item.task_id,
    course_title: item.course_name,
    test_name: item.task_name,
    test_starttime: dayJS(item.begin_time).format("YYYY-MM-DD HH:mm:ss"),
    test_endtime: dayJS(item.end_time).format("YYYY-MM-DD HH:mm:ss"),
    test_score: 0,
  }));
};
...
let getTest = async () => {
  ...
  if (back_data.status) {
    testData.value = formatData(back_data.message);
